...
public class AllTeamAreaVM extends Table {
    ...
    List<TeamNodeVM> nodes = new ArrayList<>();
    ...
    public AllTeamAreaVM(TeamScreen parent) {
        this.parent = parent;
...
//        });
    ...
    public void updateData(List<TeamPrototype> teamPrototypes) {
        ...
        for (int i = 0; i < teamPrototypes.size(); i++) {
            ...
            TeamNodeVM teamManagerVM = new TeamNodeVM(parent.getGame());
            teamManagerVM.updateData(teamPrototype);
            nodes.add(teamManagerVM);
